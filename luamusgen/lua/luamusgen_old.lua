...
function util.bpm_length(bpm)
    return 60 / bpm
end

function util.dB_amp(dB)
    return 10 ^ (dB / 20)
end

function util.amp_dB(amp)
...
function ParLinearExp(p1, v1, v2)
    p1 = verify_par(p1)
...
function ParRepeat(p1)
    ...
    if not p1.length then
        error("Parameter " .. i .. " has no length")
    end
...
function ParVibratoPulse(freq, base, exp_amp, shape, type, start_phase)
    freq = verify_par(freq)
    base = verify_par(base)
    exp_amp = verify_par(exp_amp)
    shape = verify_par(shape)
...
function ParParabolaExp(length, p1, p2, shape)
    ...
    local b = p1 - 2 * diff * (1 - 2 * shape)
    local c = (4 * shape - 1) * diff + 2 * p1 - b
    return ParLinearExp(ParLerp(0, ParLerp(0, p1, length, b), length, c), p1, p2):len(length)
end
...
function ParSmoothChangeExp(length, p1, p2)
    ...
    local c = 3 * p1 - 2 * b - 3 * diff
    local d = 3 * p1 - b - c
    return ParLinearExp(ParLerp(0, ParLerp(0, ParLerp(0, p1, length, b), length, c), length, d), p1, p2):len(length)
end

function ParLerpExp(length, v1, v2)
    return ParLinearExp(ParLerp(0, v1, length, v2), v1, v2):len(length)
end
                ...
                if (not control.const) and num then
                    if control.line then
                        ...
                        elseif control.shape then
                            ...
                            pars[i] = ParLerpExp(actual_dur, num - sustain, elem - sustain)
                        end
...
function BufferWrapper:bufferFromRaw(filename, start_point, length)
    self:endLevel()
...
function BufferWrapper:bufferFromWAV(filename, start_point, length)
    self:endLevel()
    start_point, length = self:verifyInterval(start_point, length)
...
function BufferWrapper:bufferToRaw(filename, start_point, length)
    self:endLevel()
    start_point, length = self:verifyInterval(start_point, length)
...
function BufferWrapper:bufferToWAV(filename, start_point, length)
    self:endLevel()
    start_point, length = self:verifyInterval(start_point, length)
...
function BufferWrapper:normalize(allow_volume_up, provide_feedback, start_point, length)
...
function BufferWrapper:softClipCompressor(linear_threshold, linear_amp, tanh_wet_dry, start_point, length)
...
function BufferWrapper:butterworthBandPass(iterations, freq1, freq2, resonance, wet_dry, start_point, length)
...
function BufferWrapper:butterworthBandStop(iterations, freq1, freq2, resonance, wet_dry, start_point, length)
    freq1 = verify_par(freq1)
    freq2 = verify_par(freq2)
    resonance = verify_par(resonance) or ParConst(0, freq1.sample_rate)
    wet_dry = verify_par(wet_dry) or ParConst(1, freq1.sample_rate)
...
function BufferWrapper:butterworthHighPass(iterations, freq, resonance, wet_dry, start_point, length)
    freq = verify_par(freq)
...
function BufferWrapper:butterworthLowPass(iterations, freq, resonance, wet_dry, start_point, length)
    freq = verify_par(freq)
    resonance = verify_par(resonance) or ParConst(0, freq.sample_rate)
    wet_dry = verify_par(wet_dry) or ParConst(1, freq.sample_rate)
    start_point, length = self:verifyInterval(start_point, length)
...
function BufferWrapper:flangerFeedback(freq, wet_dry, start_point, length, buffer_length)
    freq = verify_par(freq)
...
function BufferWrapper:flangerFeedforward(freq, wet_dry, start_point, length, buffer_length)
    freq = verify_par(freq)
    wet_dry = verify_par(wet_dry)
    start_point, length = self:verifyInterval(start_point, length)
    buffer_length = buffer_length or self.sample_rate
...
function BufferWrapper:flangerSelfOscillation(freq, oscil_length, correction, start_point, length, buffer_length)
    freq = verify_par(freq)
...
function BufferWrapper:phaser(stages, coefficient, feedback, wet_dry, start_point, length)
...
function BufferWrapper:moogHighPass(saturation, freq, resonance, wet_dry, inner_multiplier, start_point, length)
    freq = verify_par(freq)
    resonance = verify_par(resonance) or ParConst(0, freq.sample_rate)
    wet_dry = verify_par(wet_dry) or ParConst(1, freq.sample_rate)
...
function BufferWrapper:moogLowPass(saturation, freq, resonance, wet_dry, inner_multiplier, start_point, length)
    freq = verify_par(freq)
    resonance = verify_par(resonance) or ParConst(0, freq.sample_rate)
    wet_dry = verify_par(wet_dry) or ParConst(1, freq.sample_rate)
    inner_multiplier = verify_par(inner_multiplier) or ParConst(1, freq.sample_rate)
...
function BufferWrapper:changeVolume(multiplier, start_point, length)
...
function BufferWrapper:expShape(exponent, start_point, length)
...
function BufferWrapper:limiter(limit, start_point, length)
...
function BufferWrapper:ringMod(length, other, other_start_point, start_point)
...
function BufferWrapper:saturationTanh(start_point, length)
    start_point, length = self:verifyInterval(start_point, length)
    local node = cpp.saturationTanh(self.sample_rate, length, self.transform_mode)
    self:registerRegularTransform(node, start_point, length)
end

function BufferWrapper:simpleReverb(room_size, damping, wet_dry, space_width, rel_length, start_point, length)
...
function BufferWrapper:simpleReverseReverb(room_size, damping, wet_dry, space_width, rel_length, start_point, length)
    room_size = verify_par(room_size)
    damping = verify_par(damping)
    wet_dry = verify_par(wet_dry)
    space_width = verify_par(space_width)
...
function BufferWrapper:constantSignal(value, start_point, length)
    value = verify_par(value)
    start_point, length = self:verifyInterval(start_point, length)
...
function BufferWrapper:polysynthSwtPulse(freq_swt, freq_pulse, freq_mod_swt, freq_mod_pulse, swt_pulse_bias, pulse_shape, start_point, length, start_phase_swt, start_phase_pulse)
...
function BufferWrapper:superwaveOscillator(freq, mix, detune, type, start_point, length, start_phase, seed)
    freq = verify_par(freq)
    mix = verify_par(mix)
...
function BufferWrapper:superwaveOscillatorShaped(freq, mix, detune, shape, type, start_point, length, start_phase, seed)
    freq = verify_par(freq)
    mix = verify_par(mix)
    detune = verify_par(detune)
    shape = verify_par(shape)
...
function BufferWrapper:waveOscillator(freq, type, start_point, length, start_phase)
    freq = verify_par(freq)
    type = type or cpp.SIN
    start_point, length = self:verifyInterval(start_point, length)
    start_phase = start_phase or 0
...
function BufferWrapper:waveOscillatorShaped(freq, shape, type, start_point, length, start_phase)
    freq = verify_par(freq)
    shape = verify_par(shape)
    type = type or cpp.PULSE
    start_point, length = self:verifyInterval(start_point, length)
...
function BufferWrapper:whiteNoise(start_point, length, seed)
    start_point, length = self:verifyInterval(start_point, length)
    seed = seed or math.random(0, 2147483647)
...
function BufferWrapper:improvedStereo(pan, start_point, length, destination, dest_start_point)
    pan = verify_par(pan)
    start_point, length = self:verifyInterval(start_point, length)
    local node
    if self.bufferR then
...
function BufferWrapper:legacyStereo(pan, start_point, length, destination, dest_start_point)
    pan = verify_par(pan)
    start_point, length = self:verifyInterval(start_point, length)
    dest_start_point = dest_start_point + start_point
    local node
...
function BufferWrapper:simpleStereo(pan, start_point, length, destination, dest_start_point)
    pan = verify_par(pan)
    start_point, length = self:verifyInterval(start_point, length)
    dest_start_point = dest_start_point + start_point
    local node
...
local function LerpExp_ret(self, argument)
    return util.lerp_exp(self.v1, self.v2, argument / self.len)
end

local function LerpExp_par(self)
    return ParLinearExp(ParLerp(0, self.v1, self.len, self.v2), self.v1, self.v2)
end

function LerpExp(len, v1, v2)
    return setmetatable({
        len = len,
        v1 = v1,
        v2 = v2,
...
local function LinearExp_ret(self, argument)
    return util.map_lin_exp(self.v1, self.v2, ret_QF(self.p1, argument))
end

local function LinearExp_par(self)
    return ParLinearExp(par_QF(self.p1), self.v1, self.v2)
end

function LinearExp(p1, v1, v2)
    return setmetatable({
        p1 = p1,
        v1 = v1,
        v2 = v2,
...
local function ParabolaExp_ret(self, argument)
    ...
    return util.lerp_exp(self.v1, self.v2, a * argument * argument + b * argument)
end
...
function ConstantTimescale.new(start_point, bpm)
    return setmetatable({ start_point = start_point, beat_length = util.bpm_length(bpm) }, ConstantTimescale)
end
...
function VariableTimescale.prepareSegment(seg, current_end_point)
    local p1 = util.bpm_length(seg.bpm1)
    local p2 = util.bpm_length(seg.bpm2)
...
function Melody.new(notes)
    ...
    for i, elem in ipairs(notes) do
        if getmetatable(elem) == MelodyControl then
            ...
            local pitch, duration, start_point, note_scale, note_chord, note_defaut_scale, vars
...
function Melody:mapNoDup(map_func)
    if type(map_func) ~= map_func then
        error("expected a function")
    end
    local result_notes = {}
    for i, note in ipairs(self.notes) do
