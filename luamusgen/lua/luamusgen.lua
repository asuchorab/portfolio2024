...
if not cpp then
    error("Could not load the luamusgen C++ library, make sure you're using the luamusgen application (not a standalone lua interpreter) and that you're not running a segment file instead of a project file.")
end
...
function ParFragment(p1, offset, length)
    return ParRepeat(p1:ret(-offset)):len(length)
end
...
function Envelope(duration, segments, start_point, timescale)
    ...
    if #segs2 > 0 then
        ...
        if result then
            result = result + ParConcat(unpack(concat_tables(segs2, { ParConstMin(last_end - last_end_old) }))):offset(timescale:dur(duration, start_point) - before_len)
        else
            result = ParConcat(unpack(concat_tables(segs2, { ParConstMin(last_end - last_end_old) }))):offset(timescale:dur(duration, start_point) - before_len)
        end
    end
    if not result then
...
function BufferWrapper:isMono()
    return not self.bufferR
end

function BufferWrapper:isStereo()
    return self.bufferR
end  
...
function BufferWrapper:phaser(stages, coefficient, feedback, wet_dry, start_point, length, dont_end_level)
    ...
    feedback = verify_par(feedback) or ParConst(0, coefficient.sample_rate)
    wet_dry = verify_par(wet_dry) or ParConst(1, coefficient.sample_rate)
    start_point, length = self:verifyInterval(start_point, length)
...
function BufferWrapper:ringMod(other, other_start_point, start_point, length, dont_end_level)
    ...
    start_point, length = self:verifyInterval(start_point, length)
    other_start_point = other_start_point or 0
    if not dont_end_level then
        self:endLevel()
        other:endLevel()
    end
...
local function ret_FO(p, argument)
...
local function SetStart_ret(self, argument)
    return ret_FO(self.p1, argument - self.start)
end

local function SetStart_par(self)
    return par_FO(self.p1):offset(self.start)
end
...
local function Add_ret(self, argument)
    return ret_FO(self.p1, argument) + ret_FO(self.p2, argument)
end

local function Add_par(self)
    return ParAdd(par_FO(self.p1), par_FO(self.p2))
end
...
local function Concat_ret(self, argument)
    if argument <= 0 then
        return ret_FO(self.args[1][1], 0)
    end
    for _, arg in ipairs(self.args) do
        if argument < arg[2] then
            return ret_FO(arg[1], argument)
        else
            argument = argument - arg[2]
        end
    end
    local last_arg = self.args[#self.args]
    return ret_FO(last_arg[1], last_arg[2])
end

local function Concat_par(self)
    local pars = {}
    for i, arg in ipairs(self.args) do
        pars[i] = par_FO(arg[1]):len(arg[2])
    end
    ...
    for _, val in ipairs { ... } do
        if temp then
            if verify_num(val) then
                ...
                if getmetatable(val) == FunctionObject then
                    temp = val
...
local function ConstAt_ret(self, argument)
    return ret_FO(self.p1, argument + self.offset)
end

local function ConstAt_par(self)
    return ParConstAt(par_FO(self.p1):offset(-self.offset))
end
...
local function Div_par(self)
    return ParDiv(par_FO(self.p1), par_FO(self.p2))
end
...
local function Exp_par(self)
    return ParExp(par_FO(self.p1), par_FO(self.p2))
end
...
local function Lerp_ret(self, argument)
    ...
    return (1 - weight) * ret_FO(self.p1, argument) + weight * ret_FO(self.p2, argument)
end

local function Lerp_par(self)
    return ParLerp(self.t1, par_FO(self.p1), self.t2, par_FO(self.p2))
end
...
local function Linear_par(self)
    return ParLerp(0, par_FO(self.p1), self.len, par_FO(self.p2))
end
...
local function LinearExp_ret(self, argument)
    return lerp_exp(par_FO(self.p1), par_FO(self.p2), argument / self.len)
end

local function LinearExp_par(self)
    local p1 = ret_FO(self.p1, argument)
    local p2 = ret_FO(self.p2, argument)
...
local function LinearToExp_ret(self, argument)
    return map_lin_exp(self.v1, self.v2, ret_FO(self.p1, argument))
end

local function LinearToExp_par(self)
    return ParLinearToExp(par_FO(self.p1), self.v1, self.v2)
end
...
local function Limit_ret(self, argument)
    local value = ret_FO(self.p1, argument)
    if value > 1 then
        return 1
    elseif value < -1 then
        return -1
...
local function Mul_par(self)
    return ParMul(par_FO(self.p1), par_FO(self.p2))
end
...
local function Oscillator_par(self)
    return ParOscillator(par_FO(self.freq), par_FO(self.base), par_FO(self.amp), self.type, self.start_phase)
end
...
local function OscillatorShaped_par(self)
    return ParOscillatorShaped(par_FO(self.freq), par_FO(self.base), par_FO(self.amp), par_FO(self.shape), self.type, self.start_phase)
end
...
local function Sub_par(self)
    return ParSub(par_FO(self.p1), par_FO(self.p2))
end
...
local function Vibrato_par(self)
    return ParVibrato(par_FO(self.freq), par_FO(self.base), par_FO(self.exp_amp), self.type, self.start_phase)
end
...
local function VibratoShaped_par(self)
    return ParVibrato(par_FO(self.freq), par_FO(self.base), par_FO(self.exp_amp), par_FO(self.shape), self.type, self.start_phase)
end
...
function FunctionObject.__add(p1, p2)
    return Add(p1, p2)
end

function FunctionObject.__sub(p1, p2)
    return Sub(p1, p2)
end

function FunctionObject.__mul(p1, p2)
    return Mul(p1, p2)
end

function FunctionObject.__div(p1, p2)
    return Div(p1, p2)
end

function FunctionObject.__exp(p1, p2)
    return Exp(p1, p2)
end

function FunctionObject:offset(start)
...
function Timbre:outputNotes(buf, notes)
    ...
    if self.vars_offset and self.vars_offset ~= 0 then
        for k, v in pairs(self.vars) do
            ...
            elseif getmetatable(v) == FunctionObject then
...
function Timbre:outputRepeat(buf, offset, num_repeats, repeat_interval, melody, base, timescale, scale)
    base = base or default_base
    timescale = timescale or default_timescale
    scale = scale or default_scale
    local notes = melody:getNotes(offset, base, timescale, scale)
    local buf_temp
    if buf:isMono() then
        buf_temp = BufferWrapper.newMono()
    else
        buf_temp = BufferWrapper.newStereo()
    end
    self:outputNotes(buf_temp, notes)
    buf_temp:endLevel()
    repeat_interval = timescale:dur(repeat_interval)
    for i = 1, num_repeats do
        buf_temp:copyBuffer(buf, repeat_interval * (i - 1), ParConst(1), nil, nil, true)
    end
    buf_temp:endLevel()
end

function Timbre:outputRepeatNoCache(buf, offset, num_repeats, repeat_interval, melody, base, timescale, scale)
    base = base or default_base
    timescale = timescale or default_timescale
    scale = scale or default_scale
    for i = 1, num_repeats do
        local notes = melody:getNotes(offset + repeat_interval * (i-1), base, timescale, scale)
        self:outputNotes(buf, notes)
    end
end
...
lazy_map = {}
lazy_map.__index = lazy_map

function lazy_map.new()
    return setmetatable({
        functions = {},
        initialized = {},
    }, lazy_map)
...
function lazy_map:__newindex(k, v) 
    self.functions[k] = v
end

function lazy_map:__index(k)
    local v = self.initialized[k]
    if v then
        return v
    else
        local func = self.functions[k]
        if not func then
            return nil
        end
        local v_new = func()
        self.initialized[k] = v_new
        return v_new
    end
end

default_timescale = ConstantTimescale.new(0, 120)
