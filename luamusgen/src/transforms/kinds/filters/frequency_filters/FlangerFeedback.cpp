...
FlangerFeedback::FlangerFeedback(int64_t buffer_length) : buffer_length(buffer_length) {
  if (buffer_length <= 0) {
...
struct FlangerFeedback::FunctorMono {
...
template<class T1, class T2>
struct FlangerFeedback::FunctorStereo {
  static void call(const T1& frequency, const T2& wetDry, int64_t buffer_length,
...
void FlangerFeedback::applyMonoInPlace(double sample_rate, int64_t length, buf_t buf,
...
void FlangerFeedback::applyStereoInPlace(double sample_rate, int64_t length, buf_t bufL, buf_t bufR,
                                            const std::vector<arg_t>& arguments) {
  TRANSFORM_VERIFY_ARGS_EQUAL_LENGTH(2, "frequency, wet/dry");
