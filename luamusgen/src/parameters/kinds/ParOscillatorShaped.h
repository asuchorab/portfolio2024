...
#ifndef LUAMUSGEN_PAROSCILLATORSHAPED_H
#define LUAMUSGEN_PAROSCILLATORSHAPED_H
...
class ParOscillatorShaped : public ParGenerator {
  template<class T1, class T2, class T3, class T4>
  struct Functor {
    static std::unique_ptr<ParData>
    call(const T1& freq, const T2& shape, const T3& base, const T4& amp, double start_phase,
      ...
      if (ParData::IsConst(freq)) {
        ...
        if (freq_data.offset != 0 && freq_data.length != start_point + length) {
          logError("ParOscillatorShaped::generate", "freq data is misplaced, add argument with touch_zero = true");
      ...
      if (ParData::IsConst(freq)) {
        ...
        for (int64_t i = 0; i < length; ++i) {
          out[i] = ParData::Value(base, i) +
                   ParData::Value(amp, i) * oscillator.advance(phase_diff, ParData::Value(shape, i));
        }
      } else {
        for (int64_t i = 0; i < length; ++i) {
...
public:
  explicit ParOscillatorShaped(double start_phase) : start_phase(start_phase) {}
  ...
  std::unique_ptr<ParData> generate(const std::vector<const ParData*>& arguments, double sample_rate,
    ...
    PARAMETER_VERIFY_ARGS(4, "frequency, shape, base, amp");
    ...
    if (arguments[3]->length != length) {
      logErrorC("amp (arg #4) is expected to have the same length as the parameter");
      return std::make_unique<ParDataConst>(0, length, start_point);
    }
    ...
    if (arguments[3]->offset != start_point) {
      logErrorC("amp (arg #4) is expected to have the same start point as the parameter");
      return std::make_unique<ParDataConst>(0, length, start_point);
    }
    return ParDeduce<Functor>::template callParData<4>(*arguments[0], *arguments[1], *arguments[2], *arguments[3], start_phase,
                                              sample_rate, start_point, length);
  }

  MergeResult merge(const ParGenerator& other) override {
    if (auto p = dynamic_cast<const ParOscillatorShaped*>(&other)) {
...
#endif //LUAMUSGEN_PAROSCILLATORSHAPED_H
