...
ParDiv::generate(const std::vector<const ParData*>& arguments, double sample_rate, int64_t start_point,
                 int64_t length) {
...
ParGenerator::MergeResult ParDiv::merge(const ParGenerator& other) {
  if (auto p = dynamic_cast<const ParDiv*>(&other)) {
    return FULL;
  }
  return FAILURE;
}
