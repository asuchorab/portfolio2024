...
#ifndef LUAMUSGEN_PAROSCILLATOR_H
#define LUAMUSGEN_PAROSCILLATOR_H
...
class ParOscillator : public ParGenerator {
  template<class T1, class T2, class T3>
  struct Functor {
    static std::unique_ptr<ParData> call(const T1& freq, const T2& base, const T3& amp, double start_phase,
      ...
      if (ParData::IsConst(freq)) {
        ...
        if (freq_data.offset != 0 && freq_data.length != start_point + length) {
          logError("ParOscillator::generate", "freq data is misplaced, add argument with touch_zero = true");
      ...
      if (ParData::IsConst(freq)) {
        ...
        for (int64_t i = 0; i < length; ++i) {
          out[i] = ParData::Value(base, i) + ParData::Value(amp, i) * oscillator.advance(phase_diff);
        }
      } else {
        for (int64_t i = 0; i < length; ++i) {
...
public:
  explicit ParOscillator(double start_phase) : start_phase(start_phase) {}
  ...
  std::unique_ptr<ParData> generate(const std::vector<const ParData*>& arguments, double sample_rate,
    ...
    PARAMETER_VERIFY_ARGS(3, "frequency, base, amp");
    ...
    if (arguments[2]->length != length) {
      logErrorC("amp (arg #3) is expected to have the same length as the parameter");
      return std::make_unique<ParDataConst>(0, length, start_point);
    }
    ...
    if (arguments[2]->offset != start_point) {
      logErrorC("amp (arg #3) is expected to have the same start point as the parameter");
      return std::make_unique<ParDataConst>(0, length, start_point);
    }
  ...
  MergeResult merge(const ParGenerator& other) override {
    if (auto p = dynamic_cast<const ParOscillator*>(&other)) {
...
#endif //LUAMUSGEN_PAROSCILLATOR_H
